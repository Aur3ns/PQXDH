...
out_game "g11.cv" occ;
insert after "OH_1(" "let (subGtoG(x1p), subGtoG(x2p), subGtoG(x3p), subGtoG(x4p), x5p : kemsec, 
concatKDFInfo(x6p, x7p , x8p, x9p, x10p    )
 ) = (x1_1, x2_1, x3_1, x4_1, x5_1, x6) in";
crypto rom(H2);
...
insert after "OH(" "let (subGtoG(x1_1p), subGtoG(x2_1p), subGtoG(x3_1p), x4_1p : kemsec, concatKDFInfo(x5_1p, x6_1p , x7_1p, x8_1p, x9_1p )) = (x1, x2, x3, x4, x5) in";
crypto rom(H1);

out_game "g3.cv";

crypto gdh(gexp_div_8) [variables: secIKA0 -> a];
(* remark here, in the previous call to gdh, if we also do it on other variables, then CryptoVerif in fact fails to do the later proofs and simplifications. *)
...
crypto int_ctxt_corrupt(enc) r_42;
...
(* We always have some basic properties oj the KEM, e.g., public keys
...
fun c4(G,G,G,kdf_info):bitstring [data].
fun c5(G,G,G,G,kdf_info):bitstring [data].

letfun H5(g1:G,g2:G,g3:G, k:kemsec, i:kdf_info) = H(c4(g1,g2,g3,i),k).
letfun H6(g1:G,g2:G,g3:G,g4:G, k:kemsec, i:kdf_info) = H(c5(g1,g2,g3,g4,i),k).

equation forall g1,g1p:G, g2,g2p:G, g3,g3p:G, g4:G, i,ip:kdf_info;
       c4(g1,g2,g3,i) <> c5(g1p,g2p,g3p,g4,ip).
...
expand ROM_hash_large_5(hashkey, G, G, G, kemsec, kdf_info, emkey, H1, hashoracle, qH2).
expand ROM_hash_large_6(hashkey2, G, G ,G ,G, kemsec, kdf_info, emkey, H2, hashoracle2, qH3).

letfun H5(g1:G,g2:G,g3:G, k:kemsec, i:kdf_info,hk:hashkey) = H1(hk,g1,g2,g3,k,i).

letfun H6(g1:G,g2:G,g3:G,g4:G, k:kemsec, i:kdf_info, hk:hashkey2) = H2(hk,g1,g2,g3,g4,k,i).
...
fun concatKDFInfo(G,pkey,G,pkey,kempkey):kdf_info [data].
...
fun encodeKEM(bool,kempkey) : bitstring [data].


letfun signKEM(pk:kempkey,sk:skey) = sign(encodeKEM(true,pk),sk).
letfun checksignKEM(m:kempkey, p:pkey, s:t_sign) = checksign(encodeKEM(true,m),p,s).
...
(* We rely here on the assumption made in the spec that the encodings of public keys are disjoints. 
   This is both stated explicitly in [PQXDH], and verified in the implementation, with the encodings having a unique one byte prefix:
...
equation forall pkdh:G, pkkem:kempkey, t:bool;
       encodeEC(pkdh) <> encodeKEM(t,pkkem).
...
query Bcorrupted:bool,a0:G,as:pkey,b0:G,bs:pkey,sb:G,sb2:G,ob:G,a1:G,ob2:G,a12:G,pk1:kempkey,m:bitstring;
      inj-event(RecvWithOPK(Bcorrupted,a0,as,b0,bs,sb,ob,a1,pk1,m)) ==> inj-event(SendWithOPK(a0,as,b0,bs,sb2,ob2,a12,pk1,m)) && pow8(ob) = pow8(ob2) && pow8(a1) = pow8(a12) && pow8(sb) = pow8(sb2) && (Bcorrupted || sb = sb2)
      public_vars secb.
query Bcorrupted:bool,a0:G,as:pkey,b0:G,bs:pkey,sb:G,sb2:G,a1:G,a12:G,pk1:kempkey,m:bitstring;
      event(RecvWithoutOPK(Bcorrupted,a0,as,b0,bs,sb,a1,pk1,m)) ==> event(SendWithoutOPK(a0,as,b0,bs,sb2,a12,pk1,m)) && pow8(a1) = pow8(a12) && pow8(sb) = pow8(sb2) && (Bcorrupted || sb = sb2)
      public_vars secb.
...
=
   ...
   let info:kdf_info = concatKDFInfo(IKA,IKAsign,x_IKB,x_IKBsign,PQPKPubB) in
   let SK_opk : emkey = H6(dh1, dh2, dh3, dh4, SS, info
   ifdef(`DH',`, hk2',`')
   ) in
   ...
   (
	   ...
	   if m1 = m2 then
	   let msg = m1 in
           let cipher = enc(msg, zeroAD, SK_opk) in
...
ifdef(`DH',`, hk:hashkey',`')
   ...
   let INFOp:kdf_info = concatKDFInfo(IKAp,IKAsignp,x_IKBp,x_IKBsignp,PQPKPubBp) in   
   let SK_nopk = H5(dh1, dh2, dh3, SSp, INFOp
   ifdef(`DH',`, hk',`')
   ) in
   ...
   (
	   ...
	   if m1p = m2p then
	   let msg = m1p in
           let cipher = enc(msg, zeroAD, SK_nopk) in
...
let GenOPKThenRecv(secIKB : Z, IKB : G, secIKBsign : skey, IKBsign : pkey
  ...
  (* new SPK DH based *)   
   ...
   let info:kdf_info = concatKDFInfo(x_IKA,x_IKAsign,IKB,IKBsign,PQPKB) in
   let sk_opk = H6(dh1, dh2, dh3, dh4, ss, info
   ifdef(`DH',`, hk2',`')
   ) in

   let injbot(msg) = dec(msgenc, zeroAD, sk_opk) in
   ifdef(`KEM',`
...
',`')
  ...
  |
   ...
   let infop:kdf_info = concatKDFInfo(x_IKAp,x_IKAsignp,IKB,IKBsign,PQPKB) in   
   let sk_nopk = H5(dh1, dh2, dh3, ss,  infop
   ifdef(`DH',`, hk',`')
   ) in
   let injbot(msg) = dec(msgencp, zeroAD, sk_nopk) in
   ifdef(`KEM',`   
...
(*
All queries proved.
9.28user 0.04system 0:09.39elapsed 99%CPU (0avgtext+0avgdata 98072maxresident)k
*)
')
ifdef(`DH',`
(*
All queries proved.
69.38user 0.15system 1:09.74elapsed 99%CPU (0avgtext+0avgdata 226416maxresident)k
*)
',`')
